...
func createJumpHostDial(jumpHostConfig JumpHostConfig) (func(string, string) (net.Conn, error), error) {
	...
	jumpHostConf, err := buildSSHClientConfig(sshClientConfigOpts{
		user:       jumpHostConfig.User,
		privateKey: jumpHostConfig.PrivateKey,
	...
	// == 3:
	jumpHost := fmt.Sprintf("%s:%d", jumpHostConfig.Address, 22) // TODO 22
	// connectFunc := BastionConnectFunc("tcp", jHostHost, jHostConf, "tcp", host)
	log.Printf("[DEBUG] Connecting to jHost: %s", jumpHost)
	jumpHostClient, err = ssh.Dial("tcp", jumpHost, jumpHostConf) // TODO tcp
	if err != nil {
	...
	return func(string, string) (net.Conn, error) {
		log.Printf("--> In jump host return dial func")
		return jumpHostClient.Dial("tcp", jumpHostConfig.Address)
	}, nil
}

func connectToAgent() (*sshAgent, error) {
	// if c.Agent != true {
	// 	// No agent configured
	// 	return nil, nil
	// }
...
func readPrivateKey(pk string) (ssh.AuthMethod, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to read keyfile %q: %s", pk, err)
	}
	...
	signer, err := ssh.ParsePrivateKey([]byte(dat))
	if err != nil {
