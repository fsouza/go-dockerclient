...
func (s *DockerServer) buildMuxer() {
	...
	s.mux.Path("/networks").Methods(http.MethodPost).HandlerFunc(s.handlerWrapper(s.createNetwork))
	...
	s.mux.Path("/networks/create").Methods(http.MethodPost).HandlerFunc(s.handlerWrapper(s.networkCreate))
}
